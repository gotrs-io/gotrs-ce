...
-- CREATE INDEX IF NOT EXISTS idx_users_email ON users(email); -- Column doesn't exist

-- Customer tables indexes
CREATE INDEX IF NOT EXISTS idx_customer_user_login ON customer_user(login);
-- CREATE INDEX IF NOT EXISTS idx_customer_user_email ON customer_user(email); -- Using email from customer_user table
...
-- Tables role_user and group_role don't exist in OTRS schema
...
-- Session indexes
-- Sessions table doesn't have user_id or expires_at columns in OTRS schema
...
CREATE INDEX IF NOT EXISTS idx_ticket_closed 
    ON ticket(queue_id, change_time) 
    WHERE ticket_state_id IN (5, 6); -- Closed states (using change_time as close_time doesn't exist)
...
CREATE INDEX IF NOT EXISTS idx_article_list_covering 
    ON article(ticket_id, create_time, article_sender_type_id, create_by);
...
-- ANALYZE role_user; -- Table doesn't exist
-- ANALYZE group_role; -- Table doesn't exist
