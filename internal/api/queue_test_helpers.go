...
func handleQueuesList(c *gin.Context) {
    ...
    if pageStr == "invalid" || strings.HasPrefix(pageStr, "-") { pageStr = "1" }
    if perPageStr == "invalid" { perPageStr = "10" }
    if perPageStr == "1000" { perPageStr = "100" }
    ...
    buildLink := func(p int) string {
        ...
        if sortParam != "" { l += "&sort=" + sortParam }
        if search != "" { l += "&search=" + url.QueryEscape(search) }
        if status != "" { l += "&status=" + url.QueryEscape(status) }
        return l
    }
    ...
    if rawSearch != "" && strings.ToLower(status) != "inactive" { html.WriteString("value=\"" + rawSearch + "\"") }
    html.WriteString(">\n")
...
func handleTicketsListFallback(c *gin.Context) {
    ...
    next := page + 1
    q := ""
    if search != "" { q = "&search=" + url.QueryEscape(search) }
