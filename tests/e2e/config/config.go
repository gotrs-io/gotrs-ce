...
func detectReachableBaseURLVerbose(initial string) string {
	...
	if err == nil {
		...
		if host != "localhost" && host != "127.0.0.1" {
			for _, p := range basePorts { candidates = append(candidates, "http://localhost:"+p) }
			for _, p := range basePorts { candidates = append(candidates, "http://127.0.0.1:"+p) }
		}
		// If original host looked like backend service include its canonical variants.
		if strings.Contains(host, "backend") {
			for _, p := range []string{"8080","18080","8081"} { candidates = append(candidates, "http://backend:"+p) }
		}
	}
...
func fallbackBackendHost(current string) string {
	...
	if u.Hostname() != "backend" { return current }
	// Try backend first
