...
func (s stubGen) Next(ctx context.Context, store ticketnumber.CounterStore) (string, error) { return "45000042", nil }

// dummy store (never called because stubGen ignores it)
type dummyStore struct{}
func (d dummyStore) Add(ctx context.Context, dateScoped bool, offset int64) (int64, error) { return 0, nil }
...
func TestTicketRepositoryCreate_IncrementsTN(t *testing.T) {
    ...
    if err := repo.Create(ticket); err != nil { t.Fatalf("create failed: %v", err) }
    if ticket.ID != 123 { t.Fatalf("expected ID=123 got %d", ticket.ID) }
    if ticket.TicketNumber != "45000042" { t.Fatalf("tn mismatch got %s", ticket.TicketNumber) }
