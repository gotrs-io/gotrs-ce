...
func getCurrentEngine(defaultEngine *gin.Engine) *gin.Engine {
    if v := hotReloadableEngine.Load(); v != nil {
        if eng, ok := v.(*gin.Engine); ok && eng != nil { return eng }
    }
...
func startRoutesWatcher() {
    if os.Getenv("ROUTES_WATCH") == "" { return }
    ...
    go func() {
        ...
        for {
            ...
            filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
                ...
                if !(filepath.Ext(path) == ".yaml" || filepath.Ext(path) == ".yml") { return nil }
                mt := info.ModTime()
...
func registerYAMLRoutesManifestOnly() {
    ...
    for _, doc := range docs {
        for _, rt := range doc.Spec.Routes {
            ...
            if len(fullPath) == 0 || fullPath[0] != '/' { fullPath = "/" + fullPath }
