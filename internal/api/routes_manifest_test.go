...
func TestBuildRoutesManifestStructure(t *testing.T) {
    ...
    if err != nil { t.Fatalf("BuildRoutesManifest error: %v", err) }
    var parsed struct {
    ...
    if parsed.GeneratedAt == "" { t.Errorf("missing generatedAt timestamp") }
    if len(parsed.Routes) == 0 { t.Fatalf("expected at least one route in manifest") }
